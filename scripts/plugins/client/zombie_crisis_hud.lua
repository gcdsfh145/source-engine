...
local specialId = "zombie_crisis_special"
local teamIds = {}
...
local function directionText(heading, distance)
    if distance <= 180 then return "AT ENDPOINT" end
    if math.abs(heading) <= 25 then return "GO FORWARD" end
    if math.abs(heading) >= 155 then return "TURN AROUND" end
    return heading > 0 and "TURN RIGHT" or "TURN LEFT"
end

local function draw(mode, wave, infected, special, distance, heading, players)
    plugin.hud.create_rect(panelId, 24, 24, 520, 108 + #players * 28,
        {r = 8, g = 12, b = 18, a = 220})
    ...
    plugin.hud.create_text(specialId, 42, 120,
        string.format("Endpoint: %.0f | %s | Specials: %s", distance,
            directionText(heading, distance), special), 14,
    ...
    for i, info in ipairs(players) do
        ...
        plugin.hud.create_text(id, 42, 148 + (i - 1) * 28,
...
net.Receive("zombie_crisis_hud", function()
    ...
    draw(mode, wave, infected, special, distance, heading, players)
end)
